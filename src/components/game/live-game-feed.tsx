...
interface GameEvent {
  id: string;
  type: "player_joined" | "answer_submitted" | "phase_changed" | "round_started" | "score_awarded";
  message: string;
  player?: Player;
  timestamp: Date;
  data?: any;
...
export default function LiveGameFeed() {
  ...
  useEffect(() => {
    ...
    if (newEvents.length > 0) {
      setEvents((prevEvents: GameEvent[]) => {
        ...
        const uniqueNewEvents = newEvents.filter((e: GameEvent) => !existingIds.has(e.id));
  ...
  const getEventIcon = (type: GameEvent["type"]) => {
    switch (type) {
      ...
      case "answer_submitted": return Send;
      case "phase_changed": return Star;
      case "round_started": return Trophy;
      case "score_awarded": return Crown;
      default: return Star;
    }
  };
  ...
  const getEventColor = (type: GameEvent["type"]) => {
    switch (type) {
      ...
      case "phase_changed": return "text-purple-600";
      case "round_started": return "text-orange-600";
      case "score_awarded": return "text-yellow-600";
          ...
          <>
            {displayEvents.map((event: GameEvent) => {
              ...
              return (
                ...
                >
                  {/* Event icon */}
                  <div className={`w-8 h-8 rounded-full bg-white border-2 flex items-center justify-center ${colorClass} border-current`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {event.player && (
                      <div className="flex items-center gap-2 mt-1">
                        <div 
                          className={`w-4 h-4 rounded-full ${event.player.avatar} flex-shrink-0`}
                        />
