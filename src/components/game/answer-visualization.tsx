...
export default function AnswerVisualization() {
  ...
  if (gameState.gamePhase !== "guessing" && gameState.gamePhase !== "revealing") {
    return null;
  }
  ...
  const getAnswerStyle = (answer: Answer) => {
    ...
    let baseClasses = "p-4 rounded-lg border-2 transition-all duration-300 cursor-pointer";
  ...
  const correctGuess = selectedAnswerId && doctorAnswer && 
    answers.find((a: Answer) => a.id === selectedAnswerId)?.answer === doctorAnswer.answer;
    ...
    <Card className="border-0 shadow-xl">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          {isGuessingPhase && (
            <Badge variant={selectedAnswerId ? "default" : "secondary"} className="text-sm">
      ...
      <CardContent className="space-y-4">
        ...
        {isGuessingPhase && (
          <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
            <p className="text-purple-700 text-sm font-medium">
              🕵️ Detective Time! Click on the answer you think came from Dr. Whatchamacallit.
            </p>
          </div>
        )}
        ...
        {isRevealingPhase && (
          <div className={`border rounded-lg p-3 ${
            correctGuess ? "bg-green-50 border-green-200" : "bg-orange-50 border-orange-200"
          }`}>
        ...
        <div className="grid gap-3">
          {answers.map((answer: Answer, index: number) => {
            const player = gameState.players.find((p: Player) => p.id === answer.playerId);
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isRevealingPhase && player && (
                      <div 
                        className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold ${player.avatar}`}
                      >
